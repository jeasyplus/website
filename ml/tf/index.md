...
matplotlib代码如下，需要注意编写matplotlib代码并不是学习机器学习的重要部分。
...
也就是说，鉴于**数据集**和**超参数**，训练后的模型通常无法很好地描述特征与标签之间的关系。
...
+ 创建一个合成特征，它是总房间数与人口数量的比值。


+ 调整三个**超参数**。


+ 确定这个合成特征是否比你之前尝试过的任何单个特征产生更低的**损失值**。
...
根据损失值，这个合成特征产生的模型比你在**任务2**和**任务3**中尝试的单个特征模型更好。
...
通常情况下，**相关系数的绝对值越大**，其**预测能力就越强**。

例如，**相关系数**为 **-0.8** 的预测能力远远**优于**相关系数为 **-0.2** 的预测能力。
